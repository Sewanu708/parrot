...
const navigation = [
  { name: "Overview", href: ROUTES.DASHBOARD.ROOT, icon: InboxIcon },
  { name: "Inbox", href: ROUTES.DASHBOARD.INBOX, icon: InboxIcon },
  { name: "Contacts", href: ROUTES.DASHBOARD.CONTACTS, icon: ContactsIcon },
  {
    name: "Automations",
...
export function Sidebar() {
  ...
  const workspaces = session?.user?.tenants || [];
  ...
  const activeWorkspace =
    ...
    workspaces.find((w) => w.id === session?.user?.activeTenantId);
  ...
  return (
    <div
      className={`relative h-screen shrink-0 transition-[width] duration-300 ${isCollapsed ? "w-14" : "w-60"} z-50`}
    >
      <div
        onMouseEnter={() => {
      ...
      >
        {/* Top Section */}
        <div
          className={`p-3 space-y-4 w-full flex flex-col ${visuallyCollapsed ? "items-center px-2" : ""}`}
        >
          {/* Logo and Collapse Toggle */}
          <div
            className={`flex items-center ${visuallyCollapsed ? "justify-center w-full" : "justify-between px-2"} mb-2 mt-1`}
          >
            <Link
              href="/"
                ...
                <div className="flex items-center gap-2.5 truncate">
                  {activeWorkspace?.logoUrl ? (
                    <img
                      ...
                      className="h-5 w-5 rounded-sm object-cover shrink-0 bg-white"
                    />
                  ) : (
                  ...
                  {workspaces?.map((workspace: any) => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setIsDropdownOpen(false);
                        parrotClient.setTenantId(workspace.id);
                        await updateSession({
                          user: {
                            ...session?.user,
                            activeTenantId: workspace.id,
                          },
                        });
                        window.location.reload(); // Ensure all client states are cleared and refetched for the new tenant
                      }}
                      ...
                      <div className="flex items-center gap-2.5 truncate">
                        {workspace.logoUrl ? (
                          <img
                            ...
                            alt={workspace.name}
                            className="h-4 w-4 rounded-[3px] object-cover shrink-0 bg-white"
                          />
                        ) : (
            ...
            >
              {activeWorkspace?.logoUrl ? (
                <img
                  src={activeWorkspace?.logoUrl}
                  alt={activeWorkspace.name}
                  className="h-5 w-5 rounded-sm object-cover bg-white"
                />
              ) : (
        ...
        {/* Global Actions */}
        <div
          className={`px-3 space-y-0.5 mb-2 mt-1 w-full flex flex-col ${visuallyCollapsed ? "items-center" : ""}`}
        >
          <button
            className={`flex w-full items-center ${visuallyCollapsed ? "justify-center" : "justify-between"} gap-2.5 rounded-md ${visuallyCollapsed ? "p-2" : "px-2 py-1.5"} text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/5 text-[#37352f]/70 dark:text-[#9b9b9b] cursor-pointer outline-none group`}
          ...
          <button
            className={`flex w-full items-center ${visuallyCollapsed ? "justify-center" : "justify-between"} gap-2.5 rounded-md ${visuallyCollapsed ? "p-2" : "px-2 py-1.5"} text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/5 text-[#37352f]/70 dark:text-[#9b9b9b] cursor-pointer outline-none group relative`}
            title={visuallyCollapsed ? "Updates" : undefined}
          >
            <div
              className={`flex items-center ${visuallyCollapsed ? "justify-center" : "gap-2.5"}`}
            >
        ...
        {/* Main Navigation */}
        <nav
          className={`flex-1 space-y-0.5 px-3 mt-2 ${visuallyCollapsed ? "flex flex-col items-center" : "ml-2"}`}
        >
          {navigation.map((item) => {
        ...
        <div
          className={`p-3 border-t border-[#e9e9e7] dark:border-[#2d2d2d] flex flex-col ${visuallyCollapsed ? "items-center" : ""}`}
        >
          <div
            className={`flex items-center ${visuallyCollapsed ? "justify-center flex-col gap-3" : "justify-between gap-1"} w-full`}
          >
            <button
              className={`flex ${visuallyCollapsed ? "justify-center" : "flex-1"} items-center gap-2.5 rounded-md ${visuallyCollapsed ? "p-1" : "px-2 py-1.5"} transition-colors hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer outline-none overflow-hidden`}
              title={visuallyCollapsed ? "User Account" : undefined}
            >
              <div className="shrink-0 h-6 w-6 rounded-full bg-black/10 dark:bg-white/10 flex items-center justify-center text-[10px] font-semibold text-[#37352f] dark:text-[#ffffff] uppercase">
            ...
            <div
              className={`flex items-center gap-1 ${visuallyCollapsed ? "flex-col" : ""}`}
            >
              {mounted && (
                <button
