...
export function SettingsContent({ activeTab }: { activeTab: string }) {
  ...
  const { data: propertiesResponse } = useQuery({
    ...
    enabled: !!activeTenantId && activeTab === "properties",
  });
  
  const properties = propertiesResponse?.data || [];

  const handleCopySnippet = (property: any) => {
          ...
          {!activePropertyId ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-md bg-gray-100 dark:bg-[#252525] flex items-center justify-center shrink-0 border border-black/5 dark:border-white/5">
                          {property.logoUrl ? (
                            <img src={property.logoUrl} className="w-5 h-5 object-contain" alt="Logo" />
                          ) : (
            ...
            <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="flex flex-col">
                  ...
                  <div className="md:col-span-2">
                    <div className="bg-black border border-dash-border rounded-md p-4 overflow-x-auto relative group">
                      ...
                      <pre className="text-xs text-emerald-400 font-mono whitespace-pre-wrap word-break pt-6 sm:pt-0">
                        {activeProperty?.installationSnippet || "Loading snippet..."}
                      </pre>
                    </div>
                  </div>
                </div>
                ...
                {activeProperty?.id && <BusinessHoursSettings propertyId={activeProperty.id} />}
              </div>
            </div>
          )}
        </>
