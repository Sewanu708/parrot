  ...
  const filteredConversations =
    activePropertyFilter === "all"
      ...
      : conversations.filter(
          (c) => c.visitor.propertyId === activePropertyFilter,
        );

  const activePropertyLabel =
    activePropertyFilter === "all"
      ? "All Properties"
      : properties.find((p) => p.id === activePropertyFilter)?.name ||
        "Unknown Property";

  return (
    <div
      className={`shrink-0 border-r border-[#e9e9e7] dark:border-[#2d2d2d] bg-[#fcfcfc] dark:bg-[#1e1e1e] overflow-y-auto flex flex-col ${className}`}
    >
      {/* List Header / Filter */}
      <div
        className="px-4 py-3 border-b border-[#e9e9e7] dark:border-[#2d2d2d] flex items-center justify-between relative"
        ref={filterRef}
      >
          ...
          <div className="absolute top-full right-4 mt-1 w-48 rounded-md border border-[#e9e9e7] dark:border-[#333333] bg-white dark:bg-[#252525] p-1 shadow-lg z-50">
            <button
              ...
              className={`w-full text-left px-3 py-2 text-xs rounded-sm transition-colors cursor-pointer ${
                activePropertyFilter === "all"
                  ? "bg-black/5 dark:bg-white/5 text-[#37352f] dark:text-[#ffffff] font-medium"
                  : "text-[#37352f]/70 dark:text-[#d4d4d4] hover:bg-black/5 dark:hover:bg-white/5"
              }`}
            >
              All Properties
            </button>
            {properties.map((p) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: p.brandColor || "#4f46e5" }}
                />
                  ...
                  {property && (
                    <div className="mt-1.5 flex items-center gap-1.5">
                      <div
                        ...
                        style={{ backgroundColor: property.brandColor || "#4f46e5" }}
                      />
